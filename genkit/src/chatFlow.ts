...
import { ChatFlowInputSchema, ChatFlowOutput, ChatFlowOutputSchema, History } from './chatTypes';
import { QueryTransformFlow } from './queryTransformFlow';
import { QueryTransformFlowOutputSchema } from './queryTransformTypes';
import { ai, aiBeta } from './config';
...
`
    ...
    You are a travel agent tasked to understand the user's travel preferences and pass this information along to the (call_db_agent), if necessary.
        ...
        2. **Prioritize action:** If the user's request is broad, ask a max of 1 key clarifying questions to get a good initial understanding. Focus on essential details like:
...
// const chatSession = aiBeta.createSession<ChatState>({
//     initialState: {
//         userName: '',
//         history
//     }
...
const callDbAgent = ai.defineTool({
  name: 'call_db_agent',    
  description: "Calls the database flow to retrieve data",
