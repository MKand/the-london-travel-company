...
londen_agent_client = vertexai.Client(
    project=os.getenv("GOOGLE_CLOUD_PROJECT"),
    location=os.getenv("GOOGLE_CLOUD_LOCATION"),
)

adk_app = londen_agent_client.agent_engines.get(name=os.getenv("LONDON_AGENTENGINE_ID", "projects/n26-learn-c2c-app-dev-4/locations/us-central1/reasoningEngines/4822185870297661440"))

fake_user_client = genai.Client(
    project=os.getenv("GOOGLE_CLOUD_PROJECT"),
    location=os.getenv("GOOGLE_CLOUD_LOCATION"),
    vertexai=True
)
...
async def run_conversation(client: aiohttp.ClientSession) -> int:
    ...
    while True:
        ...
        try:
            logger.debug("\nRunning the agent...")
            response = fake_user_client.models.generate_content(
            ...
            user_message_text = f"Hi, I'm {prompt_vars['name']} and I'm interested in {prompt_vars['interest']}."
            logger.info(f"Fallback to static message: {user_message_text}")
            ...
            async for event in adk_app.async_stream_query(
                ...
                if event.content and event.content.parts:
                    for part in event.content.parts:
                        if part.text:
                            london_response_text += part.text
    ...
    while True:
        try:
            async with aiohttp.ClientSession() as client:
                turns = await run_conversation(client)
                if turns > 0:
