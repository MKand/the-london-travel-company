...
resource "google_service_account" "lta_frontend_sa" {
  account_id   = "lta-frontend-sa"
  display_name = "lta-frontend-sa"
...
resource "google_project_iam_member" "run-invoker" {
  ...
  member  = "serviceAccount:${google_service_account.lta_frontend_sa.email}"
}

resource "google_service_account" "lta_backend_sa" {
  account_id   = "lta-backend-sa"
  display_name = "lta-backend-sa"
  project      = var.gcp_project_id
}
